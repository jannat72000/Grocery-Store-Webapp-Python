from datetime import datetime
from sql_connection import get_sql_connection

def insert_order(connection, order):
    cur = connection.cursor()

    order_query = ("INSERT INTO orders "
                   "(customer_name, total, datetime) "
                   "VALUES (%s, %s, %s)")
    order_data = (order['customer_name'], order['total'], datetime.now())


    cur.execute(order_query, order_data)
    order_id = cur.lastrowid

    order_details_query = ("INSERT INTO order_details "
                           "(order_id, product_id, quantity, total_price) "
                           "VALUES (%s, %s, %s, %s) "
                           "ON DUPLICATE KEY UPDATE quantity = quantity + VALUES(quantity), total_price = VALUES(total_price)")

    order_details_data = []
    for order_detail_record in order['order_details']:
        order_details_data.append([
            order_id,
            int(order_detail_record['product_id']),
            float(order_detail_record['quantity']),
            float(order_detail_record['total_price'])
        ])

    try:
        cur.executemany(order_details_query, order_details_data)
    except mysql.connector.IntegrityError as e:
        print(f"IntegrityError: {e}")
        connection.rollback()  # Rollback if there's an error
        return None

    connection.commit()
    return order_id

def get_order_details(connection, order_id):
    cur = connection.cursor()

    query = "SELECT * from order_details where order_id = %s"

    query = "SELECT order_details.order_id, order_details.quantity, order_details.total_price, "\
            "products.name, products.price_per_unit FROM order_details LEFT JOIN products on " \
            "order_details.product_id = products.product_id where order_details.order_id = %s"

    data = (order_id, )

    cur.execute(query, data)

    records = []
    for (order_id, quantity, total_price, product_name, price_per_unit) in cur:
        records.append({
            'order_id': order_id,
            'quantity': quantity,
            'total_price': total_price,
            'product_name': product_name,
            'price_per_unit': price_per_unit
        })

    cur.close()

    return records

def get_all_orders(connection):
    cur = connection.cursor()
    query = ("SELECT * FROM orders")
    cur.execute(query)
    response = []
    for (order_id, customer_name, total, dt) in cur:
        response.append({
            'order_id': order_id,
            'customer_name': customer_name,
            'total': total,
            'datetime': dt,
        })

    cur.close()

    # append order details in each order
    for record in response:
        record['order_details'] = get_order_details(connection, record['order_id'])

    return response


if __name__ == '__main__':
    connection = get_sql_connection()
    print(get_all_orders(connection))
    #print(insert_order(connection, {
        #'customer_name': 'jannatul',
        #'total': 100.00,  # Use a hardcoded total for testing
        #'order_details': [
         #        {
          #      'product_id': 1,
          #      'quantity': 2,
             #   'total_price': 200
         #   },
         #   {
            #    'product_id': 2,
               # 'quantity': 1,
               # 'total_price': 100
            #}
        #]
   # }))

